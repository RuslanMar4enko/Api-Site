                            ...
                            <form role="form" @input="search">
                                <div class="form-group">
                                    ...
                                    <vm-form-group>
                                        ...
                                        <template scope="props">
                                            <select v-model="filter.query.type" id="props.id" class="form-control">
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                filter: new Filter({
                    url: '/api/characteristics/get-many',
                    query: {
                        id: null,
                        title_like: null,
                        type: null,
        ...
        methods: {
            search(){
                ...
                this.filter.search({holding: true}).then((data)=>{
                    this.filter.data = this.filter.populateMany(data, Characteristic)
                }).catch(()=>{

                })
            },
            addItem(){
                this.pickItem(new Characteristic())
            },
            pickItem(item){
                this.formIsDanger = false;
            ...
            deleteItem(item){
                ...
                if (item.id)
                    this.form.submit({
                        url: '/api/characteristics/delete',
                        data: {
                            id: item.id
                        }
                    }).then(() => {
